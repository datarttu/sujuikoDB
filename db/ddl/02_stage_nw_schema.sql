...
DECLARE
  cnt       bigint;
BEGIN
  IF EXISTS (SELECT * FROM sched.segments LIMIT 1) THEN
    RAISE EXCEPTION 'sched.segments is not empty, quitting transfer';
  END IF;

  IF EXISTS (SELECT * FROM sched.trip_templates LIMIT 1) THEN
    RAISE EXCEPTION 'sched.trip_templates is not empty, quitting transfer';
  END IF;

  INSERT INTO sched.trip_templates (
    ttid, route, dir, start_times, dates
  )
  SELECT ttid, route_id, direction_id, start_times, dates
  FROM stage_gtfs.trip_template_arrays
  WHERE coalesce(route_found, false) IS true
    AND start_times IS NOT NULL
    AND dates IS NOT NULL
  ORDER BY ttid;

  GET DIAGNOSTICS cnt = ROW_COUNT;
  RAISE NOTICE '% trip templates transferred from stage_gtfs.trip_template_arrays to sched.trip_templates', cnt;
  ...
  GET DIAGNOSTICS cnt = ROW_COUNT;
  RAISE NOTICE '% rows inserted into sched.segments', cnt;
